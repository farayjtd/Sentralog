...
const MENUS = [
  { key: '/(teknik-sipil)', label: '🏠 Dashboard' },
  { key: '/(teknik-sipil)/project', label: '📋 Project' },
  { key: '/(teknik-sipil)/history', label: '📁 History' },
  { key: '/(teknik-sipil)/absen', label: '📅 Absensi' },
]

export default function TeknikSipilSidebar() {
  ...
  return (
    <View style={styles.sidebar}>
      <View style={styles.profile}>
        ...
        <Text style={styles.role}>Teknik Sipil</Text>
      </View>

      {MENUS.map(menu => {
        const isActive = pathname === menu.key || (menu.key !== '/(teknik-sipil)' && pathname.startsWith(menu.key))
        return (
          <TouchableOpacity
            key={menu.key}
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#1565c0', alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  ...
  menuItemActive: { backgroundColor: '#e3f2fd', borderLeftColor: '#1565c0' },
  menuLabel: { fontSize: 13, color: '#555' },
  menuLabelActive: { color: '#1565c0', fontWeight: '600' },
