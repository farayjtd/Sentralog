...
const MENUS = [
  { key: '/(mandor)', label: '🏠 Dashboard' },
  { key: '/(mandor)/absen', label: '📅 Absensi' },
]

export default function MandorSidebar() {
    ...
    <View style={styles.sidebar}>
      <View style={styles.profile}>
        <View style={styles.avatar}>
          <Text style={styles.avatarText}>{user?.full_name?.charAt(0) ?? 'M'}</Text>
        </View>
        <Text style={styles.name} numberOfLines={1}>{user?.full_name}</Text>
        <Text style={styles.role}>Mandor</Text>
      </View>
      {MENUS.map(menu => {
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#c62828', alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  ...
  menuItemActive: { backgroundColor: '#ffebee', borderLeftColor: '#c62828' },
  menuLabel: { fontSize: 13, color: '#555' },
  menuLabelActive: { color: '#c62828', fontWeight: '600' },
